...
export default function App() {
        ...
        socket.emit('reconnect-to-room', 
          ...
          (res: any) => {
            if (res.ok) {
              ...
              } else if (res.state === 'starting' || res.state === 'question-active') {
  ...
  useEffect(() => {
    ...
    const handleDisconnect = () => {
      ...
      const session = getSession();
      if (session && session.isPlayer && view !== 'home' && view !== 'player-join') {
        setIsReconnecting(true);
      }
    };
  ...
  useEffect(() => {
    ...
    socket.on('player-disconnected', (data: { id: string; nickname: string; playerCount: number }) => {
      console.log(`Player ${data.nickname} disconnected`);
      setPlayerCount(data.playerCount);
      // Optionally update UI to show player as disconnected
    });

    socket.on('player-reconnected', (data: { id: string; nickname: string; playerCount: number }) => {
      console.log(`Player ${data.nickname} reconnected`);
      setPlayers(prev => {
        const existing = prev.find(p => p.nickname === data.nickname);
        if (existing) {
          // Update the socket ID
          return prev.map(p => p.nickname === data.nickname ? { ...p, id: data.id } : p);
        }
        return [...prev, { id: data.id, nickname: data.nickname }];
      });
      setPlayerCount(data.playerCount);
  ...
  const connectionIndicator = connectionStatus !== 'connected' && !isReconnecting && (
    <div
      style={{
        position: 'fixed',
        top: 0,
        left: 0,
        right: 0,
        backgroundColor: connectionStatus === 'reconnecting' ? '#f59e0b' : '#ef4444',
        color: 'white',
        padding: '0.5rem',
        textAlign: 'center',
        fontSize: '0.875rem',
    ...
    >
      ...
      <div style={{ textAlign: 'center' }}>
        ...
        <p style={{ opacity: 0.7, fontSize: '1rem' }}>
          {connectionStatus === 'reconnecting' ? `Attempt ${reconnectAttempt}` : 'Connection lost'}
        </p>
  ...
  if (view === 'home') {
    return (
      <>
        {connectionIndicator}
        {reconnectionOverlay}
        <HomePage onHost={() => setView('host-setup')} onJoin={() => setView('player-join')} onLibrary={() => setView('host-library')} />
      </>
    );
  }
  ...
  if (view === 'host-setup') {
    return (
      <>
        {connectionIndicator}
        {reconnectionOverlay}
        <HostSetup onCreated={handleCreateRoom} onBack={() => setView('home')} initialQuiz={savedQuizToLoad ?? undefined} />
      </>
    );
  }
  ...
  if (view === 'player-join') {
    return (
      <>
        {connectionIndicator}
        {reconnectionOverlay}
        <PlayerJoin initialCode={initialCode} onJoined={handleJoined} onBack={() => setView('home')} />
      </>
    );
  }
